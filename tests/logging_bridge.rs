//! Integration test for the `log` -> slog-scope bridge installed by
//! `logging::init_logger`.
//!
//! Runs in its own process (cargo builds each file under `tests/` as a separate
//! test binary), which sidesteps the process-global state touched by
//! `log::set_boxed_logger` and `slog_scope::set_global_logger` — calling
//! `init_logger` from unit tests sharing a process with other tests would risk
