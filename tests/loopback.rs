...
"#,
    ...
    let mut assoc = ClientAssociationOptions::new()
        ...
        .with_presentation_context(
            ...
            vec![dicom_dictionary_std::uids::EXPLICIT_VR_LITTLE_ENDIAN],
        )
    ...
    assert!(
        arrived,
        "destination never received object or queue did not drain"
    );
    assert_eq!(
        dest_scp.received.lock().unwrap().as_slice(),
        &["1.2.3.99".to_string()]
    );
}
