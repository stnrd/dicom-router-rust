//! Integration tests for `dicom_router::tls`: building rustls configs from PEM
//! files on disk.
...
fn builds_server_and_client_configs_from_pem_files_with_mtls() {
  ...
  let client_config_no_mtls = build_client_config(&pki.ca, None, None).expect("client config without mTLS material");
