...
pub async fn start_test_scp(server_cert: &Path, server_key: &Path) -> TestScp {
    ...
    let tls_cfg = dicom_router::tls::build_server_config(server_cert, server_key, None)
        .expect("test SCP TLS config");
                        ...
                        Ok(Pdu::PData { mut data }) => {
                            for dv in &mut data {
                                if dv.value_type == PDataValueType::Command && dv.is_last {
                                    let cmd =
                                        dicom_router::dimse::decode_command(&dv.data).unwrap();
                                    if dicom_router::dimse::command_field(&cmd).unwrap()
                                        == dicom_router::dimse::C_STORE_RQ
                                    {
                                        msgid = dicom_router::dimse::uint16(
                                            &cmd,
                                            dicom_router::dimse::TAG_MESSAGE_ID,
                                        )
                                        .unwrap();
                                        class = dicom_router::dimse::string(
                                            &cmd,
                                            dicom_router::dimse::TAG_AFFECTED_SOP_CLASS_UID,
                                        )
                                        .unwrap()
                                        .to_string();
                                        inst = dicom_router::dimse::string(
                                            &cmd,
                                            dicom_router::dimse::TAG_AFFECTED_SOP_INSTANCE_UID,
                                        )
                                        .unwrap()
                                        .to_string();
                                        buf.clear();
