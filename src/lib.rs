...
pub fn run(config_path: PathBuf, check_only: bool) -> i32 {
    ...
    let (log, _guard) =
        logging::init_logger(logging::parse_level(&cfg.log_level).unwrap_or(slog::Level::Info));
...
async fn async_main(cfg: config::Config, log: slog::Logger) -> i32 {
    ...
    let server_tls = match tls::build_server_config(
        &cfg.tls.server_cert,
        &cfg.tls.server_key,
    ...
    for dest in &cfg.destinations {
        let client_tls = match tls::build_client_config(
            &dest.ca_cert,
