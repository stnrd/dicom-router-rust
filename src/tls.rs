...
///
/// `cert_path`/`key_path` are this router's own certificate chain and private
/// key, presented to every connecting client regardless of SNI. When
/// `client_ca` is `Some`, mutual TLS is enforced: connecting clients must
/// present a certificate signed by one of the CAs in that bundle, or the
/// handshake fails. When `client_ca` is `None`, any client may connect without
...
/// Build the outbound TLS client configuration used to connect to a forwarding
/// destination.
///
/// `ca_cert` is the CA bundle used to verify the destination's server
/// certificate; this is always required — outbound connections are never made
/// without verification. When `client_cert`/`client_key` are both supplied,
/// they are presented to destinations that require mutual TLS; otherwise no
...
pub fn build_client_config(
  ...
  let config = match (client_cert, client_key) {
    (Some(cert_path), Some(key_path)) => {
      let certs = load_certs(cert_path)?;
      let key = load_key(key_path)?;
      builder.with_client_auth_cert(certs, key).context(RustlsSnafu)?
    }
