//! DIMSE command set builders and parsers. Command sets are always Implicit VR
//! Little Endian.
//!
//! A DIMSE command set is a small group of data elements, all in group
//! `0x0000`, carried as the command PDV of a P-DATA-TF PDU. Because it is
//! always encoded Implicit VR Little Endian, the value representation of each
//! element is *not* present on the wire — it is determined solely by the
//! element's tag, per the DICOM data dictionary (PS3.7). This module hand-rolls
//! that encoding rather than pulling in a general-purpose dataset codec, since
//! the command set vocabulary needed by a C-STORE/C-ECHO router is tiny and
//! fixed.
...
/// `CommandDataSetType` value conventionally used to mean "a data set follows
/// this command". Any value other than [`NO_DATA_SET`] means a data set
/// follows; this is the value this module emits.
...
/// A decoded command set element value. Implicit VR Little Endian carries no VR
/// on the wire, so the variant used for a given tag is fixed by the DICOM data
/// dictionary (see [`element_kind`]).
...
pub enum CommandValue {
  ...
  ULong(u32),
  /// VR UI (or similarly text-like VRs): a string, padded to even length on the
  /// wire with a trailing NUL, which is stripped back off on decode.
  Text(String),
}

/// The wire representation used for a given command set tag, absent a VR on the
/// wire.
...
/// Look up the fixed wire representation for a well-known command set tag.
/// Unrecognized tags fall back to inferring the kind from the encoded value
/// length (2 bytes -> US, 4 bytes -> UL, anything else -> text), which is good
/// enough for a router that only needs to look past elements it doesn't
/// understand.
...
/// An ordered set of DIMSE command elements (group `0x0000`). Iterating a
/// `BTreeMap` keyed by `(group, element)` naturally yields ascending tag order,
/// which is what [`encode_command`] requires.
...
impl CommandSet {
  ...
  pub fn get(&self, tag: Tag) -> Option<&CommandValue> { self.elements.get(&tag) }
}
...
/// Read the `CommandField` (0000,0100) — the DIMSE message type — out of a
/// decoded command set. Compare against [`C_STORE_RQ`], [`C_STORE_RSP`],
/// [`C_ECHO_RQ`], [`C_ECHO_RSP`], etc. to dispatch on the message type after
/// [`decode_command`].
pub fn command_field(cmd: &CommandSet) -> Option<u16> { uint16(cmd, TAG_COMMAND_FIELD) }
...
/// Encode one data element (Implicit VR Little Endian: tag, 4-byte length,
/// value) into `out`.
...
/// `CommandGroupLength` (0000,0000) element the DICOM standard requires as the
/// first element of every command set. Any `CommandGroupLength` already present
/// in `cmd` is ignored and recomputed.
pub fn encode_command(cmd: &CommandSet) -> Vec<u8> {
  let mut body = Vec::new();
  for (tag, value) in cmd.elements.iter().filter(|(tag, _)| **tag != TAG_COMMAND_GROUP_LENGTH) {
...
/// `CommandGroupLength` value itself is not validated against the remaining
/// length (it exists on the wire for compatibility with implementations that
/// read it to size a buffer, but this decoder simply consumes elements until
/// `bytes` runs out).
pub fn decode_command(bytes: &[u8]) -> Result<CommandSet, DimseError> {
  ...
  while pos < bytes.len() {
    ...
    let length = u32::from_le_bytes([bytes[pos + 4], bytes[pos + 5], bytes[pos + 6], bytes[pos + 7]]) as usize;
    pos += 8;
    ...
    // implementations; this decoder doesn't need it, but keep it for round-trip
    // fidelity.
...
mod tests {
  ...
  fn cstore_rq_roundtrip() {
    let cmd = create_cstore_rq(42, "1.2.840.10008.5.1.4.1.1.7", "1.2.3.4.5", PRIORITY_MEDIUM);
    ...
    assert_eq!(string(&decoded, TAG_AFFECTED_SOP_INSTANCE_UID), Some("1.2.3.4.5"));
    assert_eq!(
  ...
  fn cecho_rsp_roundtrip() {
    ...
    assert_eq!(uint16(&decoded, TAG_MESSAGE_ID_BEING_RESPONDED_TO), Some(99));
    assert_eq!(uint16(&decoded, TAG_STATUS), Some(STATUS_SUCCESS));
    assert_eq!(
      string(&decoded, TAG_AFFECTED_SOP_CLASS_UID),
