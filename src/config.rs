...
pub struct Config {
    /// Address to listen on for inbound DICOM TLS associations, e.g. "0.0.0.0:2762".
...
fn default_dead_letter_dir() -> PathBuf {
    PathBuf::from("/var/lib/dicom-router/dead-letter")
}
fn default_max_concurrent_associations() -> usize {
    32
}
...
impl Config {
    pub fn load(path: &Path) -> Result<Self, ConfigError> {
        ...
        let cfg: Config =
            serde_yaml::from_str(&text).map_err(|e| ConfigError::Parse { source: e })?;
        cfg.validate()?;
        Ok(cfg)
    }
    ...
    pub fn queue_dir_for(&self, destination: &str) -> PathBuf {
        self.queue_dir.join(destination)
    }
    ...
    pub fn destinations_for_source(&self, calling_ae: &str) -> Vec<&Destination> {
        self.destinations
            .iter()
            .filter(|d| {
                d.source_ae_titles.is_empty() || d.source_ae_titles.iter().any(|t| t == calling_ae)
            })
            .collect()
    }
}
...
"#;
    ...
    fn rejects_invalid_ae_title() {
        let yaml = VALID_YAML.replace(
            "ae_title: \"ROUTER\"",
            "ae_title: \"THIS_IS_WAY_TOO_LONG_AN_AE_TITLE\"",
        );
